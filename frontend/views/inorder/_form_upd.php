    ...
    <!--div class="col-md-12"-->
        <!--?= $form->field($model,'supplier_id')
            ->widget(Select2::classname(),[
                'data' => ArrayHelper::map(app\models\Supplier::find()->all(),
                        'id','name'),
        ...
        <!--?= $form->field($model, 'supplier_id')->
        dropDownList(ArrayHelper::map(\app\models\Supplier::find()->all(),'id', 'name'),
        ['prompt'=>'']) ?-->
    <!--?= $form->field($model, 'supplier_id')->textInput() ?-->
    <!--/div-->
    <div class="col-md-12">
        <?= $form->field($model, 'SupplName')->textarea(['row'=>'4', 
            'readonly' => true,
            ])
         ?>
    </div>
