...
/* @var $searchModel app\models\search\Rolelist */
/* @var $dataProvider yii\data\ActiveDataProvider */

$js=<<<JS
$(document).on("click","[data-remote]",function(e) {
    e.preventDefault();
    $("div#role_upd_edit .modal-body").load($(this).data('remote'));
});
$('#Assigs').on('hidden.bs.modal', function (e) {
  $("div#role_upd_edit .modal-body").html('');
}); 
JS;
$this->registerJs($js);
    ...
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        ...
        <div class="modal-body">
            <?= $this->render('/rolelist/_form_mod',['model' => $model]); ?>
        </div>
...
//        ],
        'captionOptions' => ['style' => 'font-size:large'],
        'filterRowOptions' => ['style' => 'line-height:12px;font-size:x-small'],
        'columns' => [
...
//            'id',
            ...
            'buttons' => [
                'upd'=>function($url,$model){
                    $url=Yii::$app->getUrlManager()->createAbsoluteUrl(['/rolelist/updatemod','id'=>$model->id]);
                    return Html::a('<span class="glyphicon glyphicon-pencil"></span>', '#role_upd_edit', [
                                'title' => 'Изменить',
                                'data-toggle'=>'modal',
                                'data-backdrop'=>false,
                                'data-remote'=>$url
...
<!--Корректировка роли------------------------------------------------------>
    ...
    Modal::begin([
        ...
        'header' => '<h3>Корректировка роли</h3>',
        ]);
    Modal::end();
    ?>
