...
//        throw new \yii\web\NotFoundHttpException('qq');
        if(Yii::$app->user->id ) {
            if(Yii::$app->user->identity->username == 'supervisor')
            {
                $searchModel = new \app\models\search\Userlist();
                $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
                return $this->redirect(['/userlist/index']);
/*                return $this->render('/userlist/index', [
...
//            throw new \yii\web\NotFoundHttpException('qq -'.$model->username.' - '.$model->password);

//            return $this->goBack();
            $user = \app\models\User::find()->where('username=:name',[':name'=>$model->username])->one();
            $role = \app\models\Rolelist::find()
                    ->leftJoin('userlist','userlist.role_id=rolelist.id')
                    ->where('userlist.user_id=:usr',[':usr'=>$user->id])
                    ->one();
